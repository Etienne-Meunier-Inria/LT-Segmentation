...
class TrAugmentFlow() :
    ...
    def __init__(self, flow_augmentation) :
        self.augs = []
        if type(flow_augmentation) == str :
            # Deal with previous versions configs
            flow_augmentation = [flow_augmentation]
    ...
    def interpret_name(self, name) :
        ...
        elif 'scaletemporal' in name:
            self.augs.append(ScaleTemporal(name))
        elif 'randomsquarecrop' in name:
            size = int(name.split('_')[-1])
            aug = T.RandomCrop(size)
    ...
    def add_specific_args(parent_parser):
        ...
        parser.add_argument('--flow_augmentation', type=str, default='none', nargs='*')
        return parser
...
class GlobalMotion :
    ...
    def step_alpha(self) :
        self.alpha += self.stepsize_alpha
        self.alpha = min(self.alpha, self.max_alpha)
        print(f'Increased alpha : {self.alpha:.2f}')
...
class ScaleTemporal :
    def __init__(self, desc) :
        ...
        alphas = {'light' : 4, 'medium' : 8, 'strong' : 10,
                  'lighti' : 1/4, 'mediumi' : 1/8, 'strongi' : 1/10}
        self.alpha = alphas[alpha]

        self.declare()

    def declare(self) :
        print(f'Augmentation {self.desc} alpha : {self.alpha}')
    ...
    def __call__(self, flow_volume) :
        ...
        return :
            flow_volume + param_flo : ( 2 T I J)
        """
        c, t, i, j = flow_volume.shape
        sign =  1 if torch.rand(1) > 0.5 else -1
        order = (torch.rand(1) > 0.5)[0].item()
        multi =  torch.linspace(sign * 1, sign * self.alpha, t, device=flow_volume.device)
        if not order :
            multi = torch.flip(multi, (0,))
        return torch.einsum('cthw,t->cthw',flow_volume, multi)
