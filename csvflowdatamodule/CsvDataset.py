...
import torchvision, re, pickle, psutil, os, flowiz, torch, pandas as pd, numpy as np, pytorch_lightning as pl
...
class FilesLoaders() :
    ...
    def load_mask(mask_path) :
        ...
        vals =  sorted(np.unique(gt))
        assert len(vals) < 20, f'Lots of different objects in the mask {mask_path} maybe an error in the format'
        assert gt.ndim == 2, f'Error in the number of dim of {mask_path}'
        gtn = np.zeros_like(gt, dtype=int)

        for idx, val in enumerate(vals) :
            gtn[gt == val] = idx
        return torch.tensor(gtn)

    @staticmethod
    def load_npy(npy_path) :
        im = torch.tensor(np.load(npy_path)) # c, i, j
        return im
...
class CsvDataset(Dataset) :
    def __init__(self,
        ...
        self.lenient_loader = lenient_loader
    ...
    def getter(self, idx, function) :
        """
        Get the resquest for index using function and return everything in a
        dictionnary ret.
        Args :
            idx : index to get the requestion in the csv file
            function : function to do the loarding ( cache / load)
        self.lenient_loader : behaviour in case a request is not available
            False (default for training) : Return ret=None if missing part of the request
            True : Return all keys but with ret[key]=None for missing keys
        Return :
            ret (dict) or None
        """
        dict_paths = self.fs.get_requests(idx, self.request)
        if not (all(dict_paths.values()) | self.lenient_loader) : return None
        ret = {}
        for key, value in dict_paths.items() :
            try :
                if isinstance(value, (np.integer, float)) :
                    ...
                    raise Exception(f'Data type of {key}:{type(value)} not handled')
            except Exception as e :
                 #print(e, value) # File does not exist
                 if self.lenient_loader : ret[key] = None
                 else : return None
        return ret
...
class CsvDataModule(pl.LightningDataModule):
    ...
    def set_transformations(self, flow_augmentation, image_augmentation, mix_augmentation, val_augment=False, **kwargs) :
        self.transforms = {}
        self.transforms['train'] = TransformsComposer(flow_augmentation, image_augmentation, mix_augmentation)
        if val_augment :
            print('Enabling Data Augmentation on validation set')
            self.transforms['val'] = TransformsComposer(flow_augmentation, image_augmentation, mix_augmentation)
        else :
