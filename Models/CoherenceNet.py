...
class CoherenceNet(LitSegmentationModel) :
    ...
    def __init__(self, img_size, len_seq, binary_method,
        ...
        self.init_regularisation_request()

    def init_regularisation_request(self) :
        """
        Add to request all elements needed for regularisation
        """
        regularisator =  Regularisation()
        for k in self.regularisation.keys() :
            self.request.update(regularisator.regs[k].get_requests())
    ...
    def init_param_model(self, img_size, len_seq, param_model) :
        ...
        ----------
            ...
            param_model (str): Type of parametric model to generate (Affine /  Quadratic )
                              + information of spline fk with suffix '_fk{int}'.

                              ( If no suffix then no spline is used in model.)
        Returns
        -------
        ...
        self.register_buffer('grid', self.ps.get_grid((T, I, J)))
    ...
    def setup_binary_method(self, binary_method) :
        ...
        """
        if binary_method in ['optimal', 'optimax', 'linear_assignment','fair'] : self.request.add('GtMask')
    ...
    def generate_binary_mask(self, batch):
        ...
        if self.binary_method == 'smallest' :
            selected_channels = (batch['Pred'] > 0.5).sum(axis=(2,3)).argmin(1)
            batch['PredMask'] = batch['Pred'][torch.arange(batch['Pred'].shape[0]),selected_channels,:,:] > 0.5
        if self.binary_method == 'exceptbiggest' :
        ...
        elif self.binary_method == 'optimal' :
            assert len(torch.unique(batch['GtMask'])) == 2, 'GtMask have to be binary optimal'
            s_channels = []
            for itr in range(batch['Pred'].shape[1]) :
                bench=[]
                for k in range(batch['Pred'].shape[1]) :
                    pdm = sum([batch['Pred'][torch.arange(batch['Pred'].shape[0]),s,:,:] > 0.5
                                             for s in s_channels + [k]]).to(torch.bool)
                    bench.append(db_eval_iou(batch['GtMask'], pdm))
                s_channels.append(torch.stack(bench).argmax(axis=0))
            batch['PredMask'] = sum([batch['Pred'][torch.arange(batch['Pred'].shape[0]),s,:,:] > 0.5
                                     for s in s_channels]).to(torch.bool)
        elif self.binary_method == 'optimax' :
            assert len(torch.unique(batch['GtMask'])) == 2, 'GtMask have to be binary for optimax'
            b, L, I, J = batch['Pred'].shape
            ...
            for itr in range(batch['Pred'].shape[1]) :
                bench=[]
                for k in range(batch['Pred'].shape[1]) :
                    pdm = sum([binmax[torch.arange(batch['Pred'].shape[0]),s,:,:] == 1.0
                                             for s in s_channels + [k]]).to(torch.bool)
                    bench.append(db_eval_iou(batch['GtMask'], pdm))
                s_channels.append(torch.stack(bench).argmax(axis=0))
            optimax_masks = sum([binmax[torch.arange(batch['Pred'].shape[0]),s,:,:] == 1.0
                                     for s in s_channels]).to(torch.bool)
            batch['PredMask'] = optimax_masks
        ...
        elif self.binary_method == 'linear_assignment' :
            argmax = batch['Pred'].argmax(1)
            bs = batch['Pred'].shape[0]
            batch['PredMask'] = torch.zeros_like(argmax, dtype=int)
            for b in range(bs) :
                annot = batch['GtMask'][b]
                seg = argmax[b]
                matjacc = np.zeros((len(torch.unique(annot)), batch['Pred'].shape[1]))
                for idxgt in torch.unique(annot) :
                    for idxseg  in torch.unique(seg) :
                        matjacc[idxgt, idxseg] = db_eval_iou(annot[None] == idxgt, seg[None]==idxseg)[0].item()
                row_ind, col_ind = linear_sum_assignment(-matjacc)
                for ri, ci in zip(row_ind, col_ind) :
                    batch['PredMask'][b][seg == ci] = ri
