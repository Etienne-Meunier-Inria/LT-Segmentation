...
class SequenceDataloader :
    ...
    def __init__(self, data_path, img_size_gen, request_fields=['Flow'], base_dir=None, max_len=1000) :
...
def save_grid(preds, path) :
    if preds.ndim == 4 :
        assert np.allclose(preds.sum(axis=0), 1.0), f'Error in {path}'
        preds = preds.argmax(0)
    if preds.max() > 7 :
        print('Warning : displaying masks with more labels than 8, visualisation might not be complete')
    plt.figure(figsize=(25,25))
    plt.tight_layout()
    plt.imshow(make_grid(preds), cmap='Set1', vmin=-1, vmax=8, interpolation='None')
    plt.axis('off')
