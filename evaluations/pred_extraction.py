...
from evaluations.utils_evals import prepare_model, SequenceDataloader, cut_cs, stitch_cs, save_grid
...
def get_model(model_dir, last_model=False,**kwargs) :
    """
...
def save_preds(preds, batch, save_dir, cs=None, save_vis=False) :
    """
    ...
    file_name =f'{str(p)}/{p.name}_cs={cs}'
    np.save(f'{file_name}.npy', preds.cpu().numpy())
    save_grid(preds.cpu().numpy(), f'{file_name}.png')


def main(model_dir, data_file, base_dir, cs, save_vis, low_memory=False, img_size=None) :
    """
    ...
    """
    backbone, hparams = get_model(model_dir, last_model=False)
    sl = SequenceDataloader('DataSplit/'+data_file+'_{}.csv',
                            img_size_gen= img_size if img_size is not None else hparams['img_size'],
                            request_fields=['Flow'], base_dir=base_dir)
    for split in ['val'] :
        for batch in tqdm(sl.get_sequence_loader(split)) :
            print(batch[f'FlowPath'])
            preds = compute_features_preds(backbone, batch, cs, low_memory=low_memory)
            save_preds(preds, batch, model_dir +'/', cs, save_vis)
...
if __name__ == '__main__' :
    ...
    for data_file in args.data_file :
        main(args.model_dir, data_file, args.base_dir, args.cut_size, args.save_vis, args.low_memory, args.img_size)
