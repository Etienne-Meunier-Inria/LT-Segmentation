...
from evaluations.utils_evals import *
from pathlib import Path
from ShapeChecker import ShapeCheck
from scipy.optimize import linear_sum_assignment
from utils.evaluations import multi_db_eval_iou, db_eval_iou
...
def main(model_dir, data_file, base_dir, snm, cs, cst, img_size_gen=None, save_vis=False) :
    ...
    for split in ['val'] :
        ...
        for batch in tqdm(sl.get_sequence_loader(split)) :
            ...
            out_path = seq_path.replace(f'_cs={cs}', f'_slm_{snm}_cs={cs}_cst={cst}')
            np.save(out_path, new_seq[0].numpy())
            if save_vis :
                save_grid(new_seq[0].cpu().numpy(), out_path.replace('.npy', '.png'))
        ...
        path_save = f'{model_dir}/{data_file}_slm_{snm}_cs={cs}_cst={cst}_{split}.csv'
        df.to_csv(path_save)
        print(f'Save Results in {path_save}')
...
if __name__ == '__main__' :
    ...
    for data_file in args.data_file :
        main(args.model_dir, data_file,  args.base_dir, args.select_mask, args.cut_size,
             args.cut_size_select, save_vis=args.save_vis)
